...
class ListaFilmes {
    ...
    void ordQuick(int esq, int dir) {
        ...
        int pivo = (esq+dir)/2;
        while (i <= j) {
            while((filmes[i].getSituacao().compareTo(filmes[pivo].getSituacao()) < 0) || ((filmes[i].getSituacao().compareTo(filmes[pivo].getSituacao()) == 0) && (filmes[i].getNome().compareTo(filmes[pivo].getNome()) < 0))) {
                i++;
            }
            while((filmes[j].getSituacao().compareTo(filmes[pivo].getSituacao()) > 0) || ((filmes[j].getSituacao().compareTo(filmes[pivo].getSituacao()) == 0) && (filmes[j].getNome().compareTo(filmes[pivo].getNome()) > 0))) {
                j--;
            }
            if (i<=j) {
                swap(i, j);
