...
class ListaFilmes {
    ...
    void ordInsercao() {
        ...
        for (int i = 1; i < contadorLista; i++) {
            tmp = filmes[i];
            j = i - 1;
            // enquanto a data do filme[i] for menor que a data [j] (j --> ja ordenado), mover j uma posicao acima
            while ((j >= 0) && (tmp.getData().compareTo(filmes[j].getData())) < 0) {
                filmes[j + 1] = filmes[j];
                j--;
            }
            // --------------------------------------- COMPARAR NOME SE DATA FOR IGUAL --------------------------------------------
            while ((j >= 0) && ((tmp.getData().compareTo(filmes[j].getData())) == 0) && ((tmp.getNome().compareTo(filmes[j].getNome())) < 0)) {
                filmes[j + 1] = filmes[j];
                j--;
            }
            filmes[j+1] = tmp;
