...
void countingSort(Filme filmes[], int contador, Filme ordenado[]);
int getMaior(Filme filmes[]);
...
int main(void)
{
    ...
    Filme ordenado[contador];
    countingSort(filmes, contador, ordenado);
    imprimir(ordenado, contador);
    return 0;
}
...
void countingSort(Filme filmes[], int contador, Filme ordenado[])
{
    ...
    int tamCount = getMaior(filmes) + 1;
    int count[tamCount];
    ...
    for (int i = 0; i < contador; count[filmes[i].duracao]++, i++);

    //Agora, o count[i] contem o numero de elemento menores ou iguais a i
    for(int i = 1; i < tamCount; count[i] += count[i-1], i++);

    //Ordenando
    for(int i = contador-1; i >= 0; ordenado[count[filmes[i].duracao]-1] = filmes[i], count[filmes[i].duracao]--, i--);

    //Copiando para o array original
    for(int i = 0; i < contador; filmes[i] = ordenado[i], i++);
}
