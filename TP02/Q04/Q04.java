...
public class Q04 {
    public static void main(String[] args) throws Exception {
        ...
        // SEGUNDA PARTE - PESQUISA BINARIA
        int comparacoes = 0;
        filmeIn = sc.nextLine();
        while (sc.hasNextLine()) {
            comparacoes += pesquisaBinaria(filmeIn, filmes);
            filmeIn = sc.nextLine();
        }
        sc.close();
    }
    ...
    public static int pesquisaBinaria (String filme, Filme filmes[]) {
        boolean teste = false;
        int comparacoes = 0;
        int esq = 0;
        int dir = filmes.length - 1;
        int meio = 0;
        while ((teste == false) && (esq <= dir)) {
            meio = esq + ((dir - esq) / 2);
            int compare = filme.compareTo(filmes[meio].getNome());
            if (filme.equals(filmes[meio].getNome())) {
                teste = true;
            }
            else if (compare < 0) {
                // filme e menor que meio
                dir = meio - 1;
            }
            else if (compare > 0) {
                // filme e maior que meio
                esq = meio + 1;
            }
        }
        if (teste) {
